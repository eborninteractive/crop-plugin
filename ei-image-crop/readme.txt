...
= 1.10.46 =
* The undersized-resolution warning now also shows the field's recommended target size in black next to the current (red) resolution, e.g. "⚠ 800 × 600 px (1200 × 800 px)".

= 1.10.45 =
* Removed the field preview image's max-height cap, now that the preview itself is full-width.

= 1.10.44 =
* Made the field's own image preview full-width.

= 1.10.43 =
* Left-aligned the live preview's pixel size caption (was centered).

= 1.10.42 =
* Uploading or selecting an SVG no longer opens the cropper - there's no raster data to crop, so it's used directly instead. The "Adjust crop" icon is also hidden for SVG-backed field values.

= 1.10.41 =
* Uploading an image already at a field's exact target pixel size no longer generates a duplicate "crop" attachment identical to the original - the original is used directly instead.

= 1.10.40 =
* Replaced the loading spinner with a custom rotating icon instead of WP core's own.
...
= 1.10.37 =
* Changed the crop canvas margin's background color to #020303 (was #090c0f).

= 1.10.36 =
* Adjusted the crop canvas margin to 45px top/bottom, 40px left/right (was 45px top, 40px on the other three sides).

= 1.10.35 =
* Adjusted the crop canvas margin to 45px top / 40px right, bottom, left (was 35px on those three sides).

= 1.10.34 =
* Adjusted the crop canvas margin to 45px top / 35px right, bottom, left (was 32px top / 20px bottom, flush left/right).

= 1.10.33 =
* Retargeted the popup view's tabs to .media-modal-content (more precise than .media-modal) and locked its spacing/no-divider styling with !important to match the grid view's rule.

= 1.10.32 =
* Grid view's tabs: removed top padding and made the row full-width again.
