...
class EB_Image_Crop_Ajax {

	public static function init() {
		add_action( 'wp_ajax_eb_image_crop_get_source', array( __CLASS__, 'get_source' ) );
		add_action( 'wp_ajax_eb_image_crop_save', array( __CLASS__, 'save' ) );
	}
	...
	protected static function check_access() {
		check_ajax_referer( 'eb_image_crop', 'nonce' );

		if ( ! current_user_can( 'upload_files' ) ) {
			wp_send_json_error( array( 'message' => __( 'You are not allowed to do this.', 'eb-image-crop' ) ), 403 );
		}
	}
	...
	public static function get_source() {
		...
		if ( ! wp_attachment_is_image( $source_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid source image.', 'eb-image-crop' ) ) );
		}

		$edit_source = EB_Image_Crop_Generator::get_edit_source( $source_id );
		if ( ! $edit_source ) {
			wp_send_json_error( array( 'message' => __( 'Could not read the source image.', 'eb-image-crop' ) ) );
		}

		$ratio = EB_Image_Crop_Generator::parse_ratio( $ratio_label );

		if ( $current_id ) {
			$stored_box = get_post_meta( $current_id, '_eb_crop_box', true );
			$box        = is_array( $stored_box ) ? EB_Image_Crop_Generator::sanitize_box( $stored_box ) : false;
		} else {
			$box = false;
		}
	...
	public static function save() {
		...
		if ( $existing_id && (int) get_post_meta( $existing_id, '_eb_crop_parent', true ) !== $source_id ) {
			$existing_id = 0;
		}

		$box = EB_Image_Crop_Generator::sanitize_box(
			array(
				'x' => $box['x'] ?? 0,
				'y' => $box['y'] ?? 0,
				'w' => $box['w'] ?? 1,
		...
		$result = EB_Image_Crop_Generator::generate( $source_id, $box, $ratio_label, $field_key, $existing_id );
